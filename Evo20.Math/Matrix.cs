...
namespace Evo20.Math
{
    public static class  Matrix
    {
        ...
        public static double[][] Inverse(this double[][] matrix)
        {
            ...
            // Build the augmented matrix.
            int numRows = matrix.GetUpperBound(0) + 1;
            var augmented = new double[numRows, 2 * numRows];
            for (int row = 0; row < numRows; row++)
            {
                for (int col = 0; col < numRows; col++)
                    augmented[row, col] = matrix[row] [col];
            ...
            // Extract the inverse array.
            var inverse = new double[numRows][];
            for (int row = 0; row < numRows; row++)
            {
                inverse[row] = new double[numRows];
                for (int col = 0; col < numRows; col++)
                {
                    inverse[row][col] = augmented[row, col + numRows];
                }
        ...
        public static double[][] Multiply(double[][] m1, double[][] m2)
        {
            if (m1[0].Length != m2.GetLength(0))
                return null;

            var multiply = new double[m1.GetLength(0)] [];
            for (var i = 0; i < multiply.Length; i++)
                multiply[i]= new double[m2[0].Length];
            for (var i = 0; i < multiply.GetLength(0); i++)
            {
                for (var j = 0; j < multiply[i].Length; j++)
                {
                    multiply[i][j] = 0;
                    for (var k = 0; k < multiply[i].Length; k++) // OR k<b.GetLength(0)
                        multiply[i][j] = multiply[i][j] + m1[i][k] * m2[k][j];
                }
            }
            return multiply;
